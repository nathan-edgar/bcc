...
void handle_event(void *ctx, int cpu, void *data, __u32 data_sz)
{
	...
	printf("%-11.6f %-14.14s %-6d %-7s %-4s %-10lld %-7d ",
		((double)e->ts - start_ts) / 1000000000,
		e->comm, e->pid, e->disk, rwbs, e->sector, e->len);
	if (env.queued)
		printf("%7.3f ", e->qdelta != -1 ?
			(double)e->qdelta / 1000000 : -1);
	printf("%7.3f\n", (double)e->delta / 1000000);
...
int main(int argc, char **argv)
{
	...
	obj->links.tp_btf__blk_account_io_start =
		bpf_program__attach(obj->progs.tp_btf__blk_account_io_start);
	err = libbpf_get_error(obj->links.tp_btf__blk_account_io_start);
	if (err) {
