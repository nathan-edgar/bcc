...
"\n"
"USAGE: futexctn [--help] [-T] [-m] [-p pid] [-t tid] [interval] [count]\n"
"\n"
"EXAMPLES:\n"
...
static int print_stack(struct syms_cache *syms_cache, struct futexctn_bpf *obj, struct hist_key *info)
{
	int i, err = 0, idx = 0, fd = bpf_map__fd(obj->maps.stackmap);
	unsigned long  dso_offset, *ip;
	...
	if (!ip) {
		fprintf(stderr, "failed to allock ip\n");
		return -1;
	}
...
static int print_map(struct syms_cache *syms_cache, struct futexctn_bpf *obj)
{
